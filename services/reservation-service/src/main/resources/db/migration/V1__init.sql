...
--
-- Nothing about this changes under Postgres. The guarantee is a composite
-- primary key, which is the most portable constraint there is - the argument in
-- the architecture doc ("correctness is a database index, not a distributed
-- lock") is engine-independent, and ConcurrentBookingTest proves it here.
--
...
CREATE TABLE outbox_event (
    ...
    -- VARCHAR(36) rather than CHAR(36) - see the note in notification-service's
    -- V1__init.sql for why Postgres CHAR does not survive the port.
    event_id      VARCHAR(36)   NOT NULL,
    event_type    VARCHAR(64)   NOT NULL,
    -- JSONB, not JSON: it validates well-formedness on write exactly as MySQL's
    -- JSON did, and stores parsed rather than re-parsing on every read. The
    -- entity maps it with @JdbcTypeCode(SqlTypes.JSON) over a plain String,
    -- which is what keeps ddl-auto=validate happy - a bare String would be
    -- inferred as varchar and fail validation against a jsonb column.
